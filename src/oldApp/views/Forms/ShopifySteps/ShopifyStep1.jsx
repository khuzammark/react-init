...
class ShopifyStep1 extends React.Component {
  onChange = async e => {
  ...
  render() {
    ...
    const menuItems = _.map(
      _.get(data, "shopifyAuths.edges", []),
      ({ node }) => (
        <MenuItem key={node.id} value={node.id}>
          {`Shopify Auth for ${node.userAuth.user.email} (created by ${node.userAuth.user.email} for ${node.org.name})`}
        </MenuItem>
      )
    );
      ...
      <Grid container spacing={24} justify="space-between" alignItems="center">
        <Grid item xs={9}>
          <FormControl fullWidth className={classes.selectFormControl}>
            <InputLabel
              htmlFor="shopifyAuthToken"
              className={classes.selectLabel}
            >
              Select Your Shopify Auth Credentials
            </InputLabel>
            <Select
              value={authStore || ""}
              inputProps={{
                name: "shopifyAuthToken",
                id: "shopifyAuthToken"
              }}
...
export default withStyles(extendedFormsStyle)(
  compose(graphql(ShopifyAuthsQuery))(withLoader(withRouter(ShopifyStep1)))
);
