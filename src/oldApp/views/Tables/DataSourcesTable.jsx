...
import { DataSourcesQuery, DeleteDataSource } from "queries/data.gql";
...
class DataSourcesTable extends React.Component {
  ...
  renderAlert = () => {
    const { alert, dataSourceId } = this.state;
    const { deleteDataSource, dataSources } = this.props;
    switch (alert) {
      case "deleteSuccess":
        return (
          <SweetAlert
            style={{ display: "block", marginTop: "-100px" }}
            title="Data Source Successfully Deleted!"
      ...
      case "confirmDelete":
        return (
          <SweetAlert
            ...
            onConfirm={async () => {
              await deleteDataSource({ variables: { id: dataSourceId } });
              this.setState(
                { alert: "deleteSuccess", authId: "" },
                dataSources.refetch
              );
            }}
            onCancel={() => this.setState({ alert: "", dataSourceId: "" })}
            confirmBtnCssClass={
  ...
  render() {
    const { classes, dataSources, history } = this.props;
    ...
    const tableData = _.map(
      _.get(dataSources, "dataSources.edges", []),
      ({ node }) => [
          ...
          <Card>
            ...
            <CardFooter>
              <Button
                ...
                onClick={() =>
                  history.push("/dashboard/admin/data-sources/new")
                }
              >
                <Forward className={classes.icons} /> Add New Data Source
              </Button>
            </CardFooter>
          </Card>
        </GridItem>
...
export default withStyles({ ...extendedTablesStyle, ...sweetAlertStyle })(
  compose(
    graphql(DataSourcesQuery, { name: "dataSources" }),
    graphql(DeleteDataSource, { name: "deleteDataSource" })
  )(withRouter(withLoader(DataSourcesTable, "linear", "dataSources")))
);
